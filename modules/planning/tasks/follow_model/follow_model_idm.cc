...
bool FollowModelIDM::Calculate(const NeighborVehicleInfo& neighbor_vehicle_info,
  ...
  double ego_speed = neighbor_vehicle_info.ego_speed;
  double exepect_speed =
      model_param.has_expect_speed()
          ? model_param.expect_speed()
          : neighbor_vehicle_info.ego_lane_ptr->lane().speed_limit();
  // 计算相对速度
  double delta_v = ego_speed - neighbor_vehicle_info.front_vehicle_speed;
  // 计算期望跟车距离
  ...
  double acc =
      model_param.accel() *
      (1.0 -
       std::pow((ego_speed / config_.expect_speed()), model_param.delta()) -
       std::pow(
           (expect_distance / neighbor_vehicle_info.front_vehicle_distance),
           2.0));
